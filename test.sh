...
docker_build() {
  ...
  docker run --rm \
    ...
    --platform $PLATFORM \
    test-runner \
    bash -c "cd volume; ls -la target; ./target/$TARGET_DIR/debug/${crate} && ldd /volume/target/$TARGET_DIR/debug/${crate} 2>&1 | grep -qE 'not a dynamic|statically linked' && echo ${crate} is a static executable"
