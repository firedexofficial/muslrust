...
docker_build() {
  ...
  docker run --rm \
    ...
    --platform $PLATFORM \
    test-runner \
    bash -c "cd volume; ./target/$TARGET_DIR/debug/${crate} && file ./target/$TARGET_DIR/debug/${crate} && file /volume/target/$TARGET_DIR/debug/${crate} 2>&1 | grep -qE 'statically linked' && echo ${crate} is a static executable"
