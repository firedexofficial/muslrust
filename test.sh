...
docker_build() {
  ...
  docker run --rm \
    ...
    --platform $PLATFORM \
    test-runner \
    bash -c "cd volume/test/${crate}; /volume/target/$TARGET_DIR/debug/${crate} && ldd target/x86_64-unknown-linux-musl/debug/${crate} 2>&1 | grep -qE 'not a dynamic|statically linked' && echo ${crate} is a static executable"
